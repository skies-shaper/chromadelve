...
function gameCreationScreen() {
    ...
    addMainMenuButton("Start", 130 + (10 * error), "#loadmenu-start", () => {
        if (!error) {
    ...
    addCheckbox("#useTutorial?", 80, 95 + (10 * error), 10)
    drawText("Play through tutorial?", 95, 105 + (10 * error))
}
...
function deleteSavesScreen() {
    ...
    if (itemToDelete > -1) {
        ...
        addGUIButton("yes", 78, 75, "#deletesaveconfirm", () => {
            deleteSave(itemToDelete)
            itemToDelete = -1
        })
        addGUIButton("no", 118, 75, "#deletesavedeny", () => {
            itemToDelete = -1
        })
        offset = 29
    }
...
function addCheckbox(id, x, y, size) {
    if (buttonEvents.indexOf(id) == -1) {
        buttonEvents.push(id)
        ...
        document.getElementById("gamewindow").addEventListener("mouseup", () => {
            if (mouseInArea((x - 3) * screenData.scale, (y - 3) * screenData.scale, (x + size + 3) * screenData.scale, (y + size + 3) * screenData.scale)) {
    ...
    if (mouseInArea((x - 1) * screenData.scale, (y - 1) * screenData.scale, (x + size + 1) * screenData.scale, (y + size + 1) * screenData.scale)) {
        screen.fillStyle = "#aaaaaa"
        screen.strokeStyle = "#aaaaaa"

    }
    screen.lineWidth = screenData.scale
    screen.filter = "none"
    screen.strokeRect(x * screenData.scale, y * screenData.scale, size * screenData.scale, size * screenData.scale)

    if (checkBoxStates[id]) {
        fillRect(x + 2, y + 2, size - 4, size - 4)
    }
    screen.fillStyle = "#ffffff"
}
